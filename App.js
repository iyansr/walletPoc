  ...
  useEffect(() => {
    if (data && ref.current) {
      const _d = {
        ...data,
        isAllowed: true,
        data: ['0xCa2D0dFC23f4f4b1ee01ed727664f807c21f4505'],
      };

      console.log('dd', _d);
      const a = ref.current.postMessage(JSON.stringify(_d));
      console.log('here');
    }
  }, [data, ref]);
    ...
    <View style={{ flex: 1 }}>
      <WebView
        ...
        onMessage={m => {
          ...
          //   console.log(JSON.stringify(m.nativeEvent.data, null, 2));
          setData(JSON.parse(m.nativeEvent.data));
        }}
        injectedJavaScriptBeforeContentLoaded={`
        (function () {
            if (typeof EthereumProvider === 'undefined') {
              window.statusAppNetworkId = 1
              var callbackId = 0;
              var callbacks = {};
              ...
              function getSyncResponse(payload) {
                if (
                  ...
                  typeof window.statusAppcurrentAccountAddress !== 'undefined'
                ) {
                  return web3SuccessResponse(payload, [
                  ...
                  return web3SuccessResponse(
                    payload,
                    window.statusAppcurrentAccountAddress,
                  );
                } else if (payload.method == 'net_version') {
                  return F(payload, window.statusAppNetworkId);
                } else if (payload.method == 'eth_chainId') {
                  return web3SuccessResponse(
                    payload,
                    '0x' + Number(window.statusAppNetworkId).toString(16),
                  );
              ...
              EthereumProvider.prototype.chainId =
                '0x' + Number(window.statusAppNetworkId).toString(16);
              ...
              EthereumProvider.prototype.removeListener = function (
                name,
                listenerToRemove,
              ) {
                if (!this._events[name]) {
                    ...
                    if (window.statusAppDebug) {
                      console.log(
                        'resolved sync method: ' +
                          method +
                          ', result: ' +
                          JSON.stringify(syncResponse.result),
                      );
                    }
              ...
              EthereumProvider.prototype.send = function (param1, param2) {
                ...
                if (typeof param1 == 'object') {
                  ...
                  throw new Error(
                    'unsupported call to send, param1: ' + param1 + ', param2: ' + param2,
                  );
                }
              };
